...
class EcoinventLCIAImporter(LCIAImporter):
    ...
    def __init__(self, biosphere_database: Optional[str] = None,dir_path:Optional[str]=None, methods_version:Optional[str]=None):
        
        """Initialize an instance of EcoinventLCIAImporter.

        Defines strategies in ``__init__`` because ``config.biosphere`` is dynamic.
        For path variable, only define the folder where the methods excel file is stored on your computer, not the full name of the excel file. 
        """
        self.strategies = [
            normalize_units,
            set_biosphere_type,
        ...
        _, self.cf_data, self.units, self.file = convert_lcia_methods_data(dir_methods=dir_path, version_methods=methods_version)
